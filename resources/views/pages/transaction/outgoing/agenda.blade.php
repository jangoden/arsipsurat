...
@section('content')
    ...
    <div class="card mb-5">
        ...
        <div class="table-responsive text-nowrap">
            <table class="table">
                <thead>
                    <tr>
                        <th>{{ __('model.letter.agenda_number') }}
                            @if ($order == 'asc')
                                <a
                                    href="{{ route('agenda.outgoing', ['order' => 'desc', 'search' => $search]) }}">
                                    <i class='bx bx-up-arrow-alt text-warning'></i>ASC
                                </a>
                            @elseif ($order == 'desc')
                                <a
                                    href="{{ route('agenda.outgoing', ['order' => 'asc', 'search' => $search]) }}">
                    ...
                    <tbody>
                        @foreach ($data as $agenda)
                            <tr>
                                ...
                                <td>
                                    <a
                                        href="{{ route('transaction.outgoing.show', $agenda) }}">{{ $agenda->reference_number }}</a>
                                </td>
                                <td>{{ $agenda->to }}</td>
    ...
    {!! $data->appends(['search' => $search, 'since' => $since, 'until' => $until,'order' => $order, 'filter' => $filter])->links() !!}
@endsection
